...
The Interpreter AI interface now opens from a bundled local HTTPS page before its online service is contacted. Slow or unavailable networks can no longer leave the app on a permanent loading spinner; a timed recovery screen offers retry and return-to-practice actions if the local page itself cannot start.
