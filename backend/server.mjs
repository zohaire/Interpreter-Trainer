...
export function createServer({router, diagnostic = () => {}, generationMs = 75000}) {
  ...
  const server = http.createServer(async (req,res) => {
    ...
    try {
      ...
      // No account is needed. Use the TCP peer for limits, never caller-supplied
      // device IDs or forwarding headers that could reset an anonymous quota.
      uid = req.socket.remoteAddress;
      if (!uid) throw new AiError('RATE_LIMITED',429);
